...
// ---------------------------------------------------------------------------
{
  ...
  // =========================================================================
  {
    ...
    eq('S20-H01-1 (H01-G) a materialised percent with zero history rows governs to 0%',
      noHistory.percentComplete, 0);
    eq('S20-H01-1 (H01-G) the materialised value is not treated as governed evidence',
      noHistory.source, 'no_governed_evidence');
    // --- H01-H: actual_quantity > 0 with ZERO approved history => no governed quantity. ---
    eq('S20-H01-2 (H01-H) a materialised quantity with zero approved history contributes nothing',
      noHistory.actualQuantity, 0);
    eq('S20-H01-2 (H01-H) the governed quantity is not the materialised one',
    ...
    eq('S20-H01-3 (H01-I) a materialised actual start is unreached without approved evidence',
      noHistory.actualStart, null);
    eq('S20-H01-3 (H01-I) a materialised actual finish is unreached without approved evidence',
      noHistory.actualFinish, null);
    eq('S20-H01-3 (H01-I) the activity is neither started nor completed', [noHistory.started, noHistory.completed], [false, false]);
    ...
    eq('S20-H01-1/2 (H01-G/H) materialised progress with no approved history earns no EV', f6Materialised.project.ev, 0);
    eq('S20-H01-1/2 (H01-G/H) it does not status the activity in F5 either',
    ...
    eq('S20-H01-4 (H01-J) an approved in-period update still governs the percent', governedByEvidence.percentComplete, 35);
    eq('S20-H01-4 (H01-J) ... and is identified as governed evidence', governedByEvidence.source, 'approved_update');
    eq('S20-H01-4 (H01-J) ... and cites the row it came from', governedByEvidence.evidenceUpdateId, 'HU-G');
    // An approved row after the Data Date is not evidence: the same activity stays unreached.
    eq('S20-H01-5 (H01-J) an approved row AFTER the Data Date leaves the activity unreached',
    ...
    eq('S20-H01-5 (H01-J) a submitted-only history leaves the activity unreached',
  ...
  // =========================================================================
  {
    ...
    eq('S20-H02-2 (H02-F) a different cost_type is a different business key',
      costTransactionBusinessKey(directRow) === costTransactionBusinessKey(indirectRow), false);
    eq('S20-H02-2 (H02-F) both postings survive de-duplication',
      dedupeByBusinessKey([directRow, indirectRow]).unique.length, 2);
    eq('S20-H02-2 (H02-F) both reach AC (a direct and an indirect posting are two charges)',
    ...
    eq('S20-H02-3 (H02-G) a different category is a different business key',
      costTransactionBusinessKey(workRow) === costTransactionBusinessKey(materialRow), false);
    eq('S20-H02-3 (H02-G) a category split survives de-duplication',
      dedupeByBusinessKey([workRow, materialRow]).unique.length, 2);
    eq('S20-H02-3 (H02-G) a category split is counted once per category in AC',
    ...
    eq('S20-H02-4 (H02-H) a different WBS node is a different business key',
      costTransactionBusinessKey(wbsA) === costTransactionBusinessKey(wbsB), false);
    eq('S20-H02-5 (H02-H) a different budget line is a different business key',
      costTransactionBusinessKey(lineA) === costTransactionBusinessKey(lineB), false);
    eq('S20-H02-4 (H02-H) the same cost allocated to two WBS branches is two charges',
      dedupeByBusinessKey([wbsA, wbsB]).unique.length, 2);
    eq('S20-H02-5 (H02-H) the same cost split across two budget lines is two charges',
      dedupeByBusinessKey([lineA, lineB]).unique.length, 2);
    eq('S20-H02-4/5 (H02-H) a split allocation is fully counted in AC',
    ...
    eq('S20-H02-1 (H02-I) an exact duplicate carries the same business key',
      costTransactionBusinessKey(trueDuplicate), costTransactionBusinessKey(directRow));
    eq('S20-H02-1 (H02-I) an exact duplicate is collapsed to one row',
      dedupeByBusinessKey([directRow, trueDuplicate]).unique.length, 1);
    eq('S20-H02-1 (H02-I) the later row is the one reported as dropped',
      dedupeByBusinessKey([directRow, trueDuplicate]).duplicates[0].row.id, 'S20I2');
    eq('S20-H02-1 (H02-I) an exact duplicate adds nothing to AC',
    ...
    eq('S20-H02-1 (H02-I) case and whitespace normalize to the same key',
    ...
    eq('S20-H02-1 (H02-I) the same invoice in another project is not a duplicate',
    ...
    eq('S20-H02-6 (H02-J) AC is unchanged by the duplicate exclusion', duplicated.project.ac, singleRow.project.ac);
    eq('S20-H02-6 (H02-J) CPI is unchanged by the duplicate exclusion', duplicated.project.cpi, singleRow.project.cpi);
    eq('S20-H02-6 (H02-J) EAC is unchanged by the duplicate exclusion', duplicated.project.eac, singleRow.project.eac);
    eq('S20-H02-6 (H02-J) VAC is unchanged by the duplicate exclusion', duplicated.project.vac, singleRow.project.vac);
    eq('S20-H02-6 (H02-J) the transaction count is unchanged too', duplicated.project.acCount, singleRow.project.acCount);
    ok('S20-H02-6 (H02-J) the duplicate is still reported, exclusion is not silence',
    ...
    ok('S20-H02-6 (H02-J) a legitimately distinct posting is not reported as a duplicate',
  ...
  // =========================================================================
  {
    ...
    eq('S20-M01-2 (M01-B) a null data_date is FALLBACK_DEFAULT_DATE', nullDd.source, FALLBACK_DEFAULT_DATE);
    eq('S20-M01-2 (M01-B) a null data_date is flagged as a fallback', nullDd.isFallback, true);
    ...
    eq('S20-M01-2 (M01-E) the report still resolves a usable Data Date', m01FallbackReport.dataDate, DD);
    eq('S20-M01-2 (M01-E) ... but its provenance is the governed default', m01FallbackReport.dataDateSource, FALLBACK_DEFAULT_DATE);
    eq('S20-M01-2 (M01-E) ... and the report says so explicitly', m01FallbackReport.dataDateIsFallback, true);
    // The screen renders that provenance: the flags come from the report, and both markers appear.
    ok('S20-M01-3 (M01-E) the Executive Report reads provenance from the canonical report',
      /costReport\?\.dataDateIsFallback/.test(m01ExecSrc));
    ok('S20-M01-3 (M01-E) the Executive Report renders the fallback marker',
      /dataDateIsFallback[\s\S]{0,300}FALLBACK_DEFAULT_DATE/.test(m01ExecSrc)
      && /dataDateIsFallback[\s\S]{0,400}افتراضي/.test(m01ExecSrc));
    ok('S20-M01-3 (M01-E) the Executive Report also labels the governed case',
      /EXPLICIT_GOVERNED_DATE/.test(m01ExecSrc));

    // --- M01-F: engine / report metadata preserves FALLBACK_DEFAULT_DATE. ---
    eq('S20-M01-4 (M01-F) F6 metadata preserves FALLBACK_DEFAULT_DATE',
      m01FallbackReport.dataDateSource, FALLBACK_DEFAULT_DATE);
    ok('S20-M01-4 (M01-F) the reported source is the documented constant, not a string that merely looks like it',
    ...
    eq('S20-M01-4 (M01-F) F5 metadata preserves FALLBACK_DEFAULT_DATE', m01FallbackF5.dataDateSource, FALLBACK_DEFAULT_DATE);
    eq('S20-M01-4 (M01-F) F5 also flags it as a fallback', m01FallbackF5.dataDateIsFallback, true);
    // A snapshot built from that report preserves it too (it stores the report's Data Date).
    eq('S20-M01-4 (M01-F) the snapshot built from the report carries the same date',
    ...
    eq('S20-M01-1 (M01-G) an explicit project data_date is EXPLICIT_GOVERNED_DATE',
      m01GovernedReport.dataDateSource, EXPLICIT_GOVERNED_DATE);
    eq('S20-M01-1 (M01-G) ... and is not flagged as a fallback', m01GovernedReport.dataDateIsFallback, false);
    eq('S20-M01-1 (M01-G) F5 agrees for the same project',
    ...
    eq('S20-M01-1 (M01-G) the shipped pilot project carries its own data_date',
      typeof m01Pilot.data_date, 'string');
    eq('S20-M01-1 (M01-G) the shipped pilot project is governed, not fallback',
      resolveDataDateProvenance(m01Pilot).source, EXPLICIT_GOVERNED_DATE);

    // --- M01-H: resolveDataDate compatibility is intact. ---
    eq('S20-M01-5 (M01-H) resolveDataDate still returns the same value for a governed project',
      resolveDataDate(m01GovernedProject), '2026-09-01');
    eq('S20-M01-5 (M01-H) resolveDataDate still returns the default for a project with none',
      resolveDataDate(m01FallbackProject), DEFAULT_DATA_DATE);
    eq('S20-M01-5 (M01-H) isFallbackDataDate still works', isFallbackDataDate(m01FallbackProject), true);
    eq('S20-M01-5 (M01-H) isFallbackDataDate is false for a governed project',
      isFallbackDataDate(m01GovernedProject), false);
    eq('S20-M01-5 (M01-H) the report Data Date is the caller-resolved date, provenance changes no value',
  ...
  // =========================================================================
  {
    ...
    eq('S20-H04-1 (H04-D) an unmeasured CPI publishes no realistic EAC (N/A)', gated.cost.realistic, null);
    eq('S20-H04-1 (H04-D) ... no optimistic EAC', gated.cost.optimistic, null);
    eq('S20-H04-1 (H04-D) ... no pessimistic EAC', gated.cost.pessimistic, null);
    eq('S20-H04-1 (H04-D) the budget is NOT published as the EAC', gated.cost.realistic, null);
    ...
    eq('S20-H04-2 (H04-E) forecast.measured = false prevents a numeric scenario EAC',
      dashboardScenarioEac(gated, 12345), null);
    eq('S20-H04-2 (H04-E) an anomalous index is equally barred', dashboardScenarioEac(anomalous, 0), null);
    // The card builder drops the scenario entirely rather than showing a number.
    eq('S20-H04-2 (H04-E) the EAC presentation publishes no scenario card',
    ...
    ok('S20-H04-4 (H04-F) a measured forecast still publishes a numeric realistic EAC',
      typeof measured.cost.realistic === 'number');
    eq('S20-H04-4 (H04-F) the measured EAC is AC + (BAC x remaining) / CPI, not BAC',
      measured.cost.realistic, 50000 + (100000 * 0.5) / 1);
    eq('S20-H04-4 (H04-F) the optimistic scenario is at most the realistic one',
      (measured.cost.optimistic as number) <= (measured.cost.realistic as number), true);
    eq('S20-H04-4 (H04-F) the pessimistic scenario is at least the realistic one',
      (measured.cost.pessimistic as number) >= (measured.cost.realistic as number), true);
    eq('S20-H04-4 (H04-F) the Dashboard gate lets a measured EAC through',
      dashboardScenarioEac(measured, 0), measured.cost.realistic);
    ok('S20-H04-4 (H04-F) a measured EAC still renders a scenario card',
      buildEacPresentation({
        canonicalEac: 100, canonicalMethod: null, canonicalBac: 100, canonicalVac: 0,
    ...
    eq('S20-H04-3 (H04-G) the non-nullable adapter still substitutes BAC for eac (the trap)', adapterEac, 0);
    ok('S20-H04-3 (H04-G) the unmeasured forecast never publishes that compatibility value',
      gated.cost.realistic !== adapterEac && gated.cost.realistic === null);
    eq('S20-H04-3 (H04-G) the two EAC channels stay separate: canonical null vs scenario null',
...
// ---------------------------------------------------------------------------
{
  ...
  // =====================================================================
  {
    ...
    for (const [label, es] of [['zero activities', esEmptyActivities], ['no project', esNoProject]] as const) {
      ...
      eq(`S21-NG04-1 (NG04-E) ${label}: CPI is not the synthetic 1.0`, es.costPerformanceIndex === 1, false);
      eq(`S21-NG04-2 (NG04-E) ${label}: SPI(t) is not the synthetic 1.0`, es.schedulePerformanceIndexTime === 1, false);
      eq(`S21-NG04-1 (NG04-E) ${label}: CPI is null (N/A)`, es.costPerformanceIndex, null);
      eq(`S21-NG04-2 (NG04-E) ${label}: SPI(t) is null (N/A)`, es.schedulePerformanceIndexTime, null);
      eq(`S21-NG04-2 (NG04-E) ${label}: the traditional comparison SPI is null too`,
        es.comparisonWithTraditionalEvm.evmSpi, null);
      eq(`S21-NG04-2 (NG04-E) ${label}: the ES comparison SPI is null too`,
        es.comparisonWithTraditionalEvm.esmSpi, null);
      // --- NG04-F: the status is not on_track. ---
      eq(`S21-NG04-3 (NG04-F) ${label}: status is not on_track`, es.status === 'on_track', false);
      ok(`S21-NG04-3 (NG04-F) ${label}: no positive performance state is claimed`,
        es.status !== 'ahead' && es.status !== 'on_track');
      // --- NG04-G: an explicit unmeasured / no-data state. ---
      eq(`S21-NG04-4 (NG04-G) ${label}: status is the explicit unmeasured state`, es.status, 'unmeasured');
      eq(`S21-NG04-4 (NG04-G) ${label}: the result says it is not measured`, es.measured, false);
      eq(`S21-NG04-4 (NG04-G) ${label}: no forecast completion date is invented`, es.forecastCompletionDate, null);
      ok(`S21-NG04-4 (NG04-G) ${label}: the note states there is nothing to compute`,
      ...
      eq(`S21-NG04-4 (NG04-G) ${label}: the reconciliation reports the ES trend as unavailable`,
        emptyRecon.esAvailability, 'es_unmeasured');
      eq(`S21-NG04-4 (NG04-G) ${label}: ... and publishes no trend finish`, emptyRecon.esTrendFinish, null);
      eq(`S21-NG04-4 (NG04-G) ${label}: ... and is not computable`, emptyRecon.esComputable, false);
    }

    // --- NG04-H: a REAL measured 1.0 remains valid. ---
    const esMeasured = calculateEarnedSchedule(s21EsSources);
    eq('S21-NG04-5 (NG04-H) a real run is flagged as measured', esMeasured.measured, true);
    ok('S21-NG04-5 (NG04-H) a real run publishes numeric CPI / SPI(t)',
      typeof esMeasured.costPerformanceIndex === 'number'
      && typeof esMeasured.schedulePerformanceIndexTime === 'number');
    ok('S21-NG04-5 (NG04-H) a real run publishes a real status, never the unmeasured one',
      esMeasured.status !== 'unmeasured');
    ok('S21-NG04-5 (NG04-H) a real run publishes a forecast completion date',
    ...
    eq('S21-NG04-5 (NG04-H) a genuinely measured CPI of 1.0 is still published as 1',
      perfect.costPerformanceIndex, 1);
    ok('S21-NG04-5 (NG04-H) ... and is distinguishable from the empty no-data state',
